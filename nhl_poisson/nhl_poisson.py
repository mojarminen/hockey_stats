...
DATAFILES = [
        'nhl_regular_season_2016-2017.csv',
    ]
...
def read_data(filepath):
    ...
    with open(filepath, 'rb') as csvfile:
        ...
        for row in csvreader:
            ...
            # parse result
            if 'CAN.' in row[1] or 'POSTP.' in row[1]:
                cancelled = True
                continue
            elif 'AWA.' in row[1]:
...
if __name__ == '__main__':
    ...
    get_estimations('Boston Bruins', 'New York Islanders', games)
    get_estimations('Columbus Blue Jackets', 'Los Angeles Kings', games)
    get_estimations('New Jersey Devils', 'Nashville Predators', games)
    get_estimations('Pittsburgh Penguins', 'New York Rangers', games)
    get_estimations('Florida Panthers', 'Buffalo Sabres', games)
    get_estimations('Montreal Canadiens', 'Anaheim Ducks', games)
    get_estimations('Tampa Bay Lightning', 'Detroit Red Wings', games)
    get_estimations('Minnesota Wild', 'Colorado Avalanche', games)
    get_estimations('Chicago Blackhawks', 'Ottawa Senators', games)
    get_estimations('Dallas Stars', 'St. Louis Blues', games)
    get_estimations('Vancouver Canucks', 'Winnipeg Jets', games)
    get_estimations('San Jose Sharks', 'Calgary Flames', games)
